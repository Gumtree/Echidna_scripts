...
def correctGeometryjv(ds, radius, thetaVect, Zpvertic, contribs):

    # Use the built-in Java routine
    cg = GeometryCorrection()
    dsjv = ds.storage.__iArray__.getArrayUtils().copyToNDJavaArray()
    zpjv = Zpvertic.__iArray__.getArrayUtils().copyTo1DJavaArray()
    varjv = ds.var.__iArray__.getArrayUtils().copyToNDJavaArray()
    contjv = contribs.storage.__iArray__.getArrayUtils().copyToNDJavaArray()
    cg.correctGeometry(dsjv, radius, thetaVect, zpjv, varjv, contjv)
    print("Finished correcting geometry")
    print("decurved in" + `cg.decurved_data`)

...
a familiar screen. Login details, if needed, are provided in the printed manual.
...
The following procedure will restart the Analysis window over the top of the Experimental window. To switch back to the 
experimental window, click on the appropriate name at the top right of the window (e.g. 'Low Temperature').
1. To the left of the `Customised temperature` icon on the Experimental window is a vertical bar and then a window icon
with a gold star. Click on this icon and choose 'Analysis Scripting'.
2. Click the `Load Script` button and pick `EchidnaReductionUI.py`
